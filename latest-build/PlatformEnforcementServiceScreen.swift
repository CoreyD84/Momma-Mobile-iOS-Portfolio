    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 15) {
                ...
                Text("Platform enforcement service destroyed").font(.subheadline).foregroundColor(.secondary)
                Text("UsageStatsManager not available, cannot enforce app blocking.").font(.subheadline).foregroundColor(.secondary)
                Text("Error in app enforcement loop").font(.subheadline).foregroundColor(.secondary)
                Button(action: {}) {
                    HStack {
                        Text("_alt")
                        Spacer()
