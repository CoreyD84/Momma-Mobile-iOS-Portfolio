    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 15) {
                Text("NettieDeviceAdminReceiver").font(.title2).bold().padding(.bottom)
                Button(action: {}) {
                    HStack { Text("NettieDeviceAdmin"); Spacer(); Image(systemName: "chevron.right.circle") }
                ...
                Button(action: {}) {
                    HStack { Text("Password changed"); Spacer(); Image(systemName: "chevron.right.circle") }
                    .padding().background(Color.blue.opacity(0.1)).cornerRadius(8)
                }
                Button(action: {}) {
                    HStack { Text("Password failed"); Spacer(); Image(systemName: "chevron.right.circle") }
                    .padding().background(Color.blue.opacity(0.1)).cornerRadius(8)
                }
                Button(action: {}) {
                    HStack { Text("Password succeeded"); Spacer(); Image(systemName: "chevron.right.circle") }
                    .padding().background(Color.blue.opacity(0.1)).cornerRadius(8)
                }
            }.padding()
        }
