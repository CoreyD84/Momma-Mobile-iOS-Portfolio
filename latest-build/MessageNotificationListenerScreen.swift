    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 15) {
                ...
                Text("❗️ CRITICAL threat detected but response is on cooldown").font(.subheadline).foregroundColor(.secondary)
                Text("🚨 Initiating emergency response...").font(.subheadline).foregroundColor(.secondary)
                Button(action: {}) {
                    HStack {
                        Text("category")
                        Spacer()
